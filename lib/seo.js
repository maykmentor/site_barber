...
function deriveSeo(config, origin, options = {}) {
    ...
    const configuredSocialImage = safeUrl(cleanText(seo.social_image_url), canonical);
    const socialImage = configuredSocialImage
        || safeUrl(cleanText(config?.hero?.bg_image), canonical)
        || safeUrl(cleanText(general.logo_url), canonical);
    const storageBaseUrl = safeUrl(options.supabaseUrl || process.env.SUPABASE_URL, canonical);
    const socialImageManaged = Boolean(configuredSocialImage
        && storageBaseUrl
        && new URL(configuredSocialImage).origin === new URL(storageBaseUrl).origin
        && /\/storage\/v1\/object\/public\/seo-assets\//.test(new URL(configuredSocialImage).pathname));
...
function renderSeoHead(seo) {
    const imageMetadataTags = seo.socialImageManaged ? [
